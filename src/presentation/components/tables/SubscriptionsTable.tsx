...
import SubscriptionCard from "@/presentation/components/form/SubscriptionCard.tsx";
...
export default function SubscriptionsTable() {
    ...
    const markOpen = (subscription: Subscription) => {
        openModal();
        setSelectedSubscription(subscription);
    }

    const deleteSelectedSubscription = () => {
    ...
    const toggleSubscription = async (subscription: Subscription, isActive: boolean) => {
        await updateSubscription(subscription, { isEnabled: isActive });
    }
            ...
            {subscriptions.length ? (
                ...
                <div className="w-full h-full my-20 flex items-center flex-wrap justify-center gap-10">
                    <div className="grid gap-4 w-60">
                        ...
                        <div>
                            <h2 className="text-center text-black text-base font-semibold leading-relaxed pb-1"> Ups! Aún hay suscripciones </h2>
                ...
                <div
                    ...
                    <div>
                        <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
                            ¿Realmente desea eliminar la suscripción de <strong>{selectedSubscription?.email}</strong>?
                        </h4>
                        <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                            Si haces esto dejarás de ver esta suscripción en la pagina principal
                        </p>
                    </div>
                    <div className="flex items-center justify-center gap-3 px-2 mt-6 w-full">
                        <Button variant='danger' onClick={deleteSelectedSubscription} size="sm">
                            Eliminar
                        </Button>
                    </div>
                </div>
